...
void vTaskMqtt(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (ctx->mqtt.conn_event) {
            ...
            printf("MQTT: connection status=%d  connected=%d\n",
                   ctx->mqtt.conn_status, ctx->mqtt.connected);
        }
        ...
        if (!ctx->mqtt.connected) {
            ...
            if ((now - ctx->mqtt.last_attempt) > pdMS_TO_TICKS(3000)) {
                ...
                mqtt_app_connect_once(&ctx->mqtt);
            }
            vTaskDelay(pdMS_TO_TICKS(200));
            continue;
        }
        ...
        if (ctx->mqtt.need_subscribe) {
            ...
            mqtt_app_subscribe_cmd(&ctx->mqtt);
        }

        // comando RX
        char topic_local[96];
        char payload_local[256];
        if (mqtt_app_take_cmd(&ctx->mqtt, topic_local, sizeof(topic_local),
        ...
        if (!mqtt_app_publish_frame(&ctx->mqtt, payload, (size_t)strlen(payload), frame.seq, frame.tick)) {
            // força reconexão na próxima iteração
