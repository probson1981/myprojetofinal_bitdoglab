...
static bool aht10_write_command(AHT10_Handle *dev, uint8_t cmd, uint8_t arg1, uint8_t arg2) {
    ...
    return dev->iface.i2c_write(dev->iface.i2c_port, AHT10_I2C_ADDRESS, buf, 3) == 0;
}
...
bool AHT10_SoftReset(AHT10_Handle *dev) {
    ...
    bool ok = dev->iface.i2c_write(dev->iface.i2c_port, AHT10_I2C_ADDRESS, &cmd, 1) == 0;
    dev->iface.delay_ms(20);
    return ok;
}
...
bool AHT10_IsBusy(AHT10_Handle *dev) {
    uint8_t status = 0;
    if (dev->iface.i2c_read(dev->iface.i2c_port, AHT10_I2C_ADDRESS, &status, 1) != 0) return true;
...
bool AHT10_ReadTemperatureHumidity(AHT10_Handle *dev, float *temperature, float *humidity) {
    ...
    uint8_t raw[6];
    if (dev->iface.i2c_read(dev->iface.i2c_port, AHT10_I2C_ADDRESS, raw, 6) != 0) return false;
